...
from pyrogram.raw.types import InputChatlistDialogFilter, InputPeerUser
from database import *
from data import *
from configs import *
from pyrogram.utils import get_peer_id
from pyrogram.raw.base import InputUser, InputPeer
from pyrogram.raw.functions.messages import AddChatUser, StartBot

opreat      = sys.argv[1];
opreats    = ['join','left','likee','like','Poll','copy','check','reaction','ass','leftgroup','leftchannels','leavess','joining','CX','View','joinlist','leavelist'];
...
async def start_bot(acc, link, user, idd):
	a = await acc.get_me()
	id = a.id
	peers = await acc.resolve_peer(id)
	try:
		await acc.invoke(
			StartBot(bot=InputUser(user),
			   peer=peers,
			   random_id=idd,
			   start_param=link)
		)
	except Exception as e:
			sendMessage(idd, f" error {e}")


async def Start_Bot(acc, user, idd):
	p = await acc.send_message(user, f"/start {idd}")
	w = await acc.get_messages(user, message_ids=int(p.id), replies=1)
...
async def Controll():
	global app
	try:
		app	  = Client(f"sessions/{ses}",api_id=api_id, api_hash=api_hash, lang_code="ar");		connect  = await app.connect();
	...
	if opreat == 'bot':
		link = str(sys.argv[3])
		Userr = str(sys.argv[4])
		id = sys.argv[5]
		owner = int(sys.argv[6])
		try:
			await app.send_message(Userr, f"/start {id}")
			sleep(2)
		except Exception as e:
