...
from pyrogram.raw.base import Authorization
import asyncio
import json
import configparser
import requests
...
yy = "تم تسجيل الدخول بنجاح ولكن للحصول على النقاط يرجى تسجيل الخروج من الحساب وانهاء اي جلسة اخرى عدا جلسة البوت ثم ارسل /Check لكي أتحقق .";
xx = "خطا ، لم تقم بتسجيل الخروج من جميع الجلسات !";
ss = "**تم تأكيد الحساب بنجاح واضافة النقاط الى حسابك ✅**";
...
app.connect();
#appa            = Client(name=f"sessions/{ses}",api_id=api_id, api_hash=api_hash);
#appa.connect();
...
async def signAccount():
	...
	try:
		...
		if RESPONSE == '[400 PHONE_NUMBER_BANNED]':
			delete(user_id);
			sendMessage(user_id,textBanned);
			return
		elif RESPONSE == '[420 FLOOD_WAIT_X]':
			delete(user_id);
			sendMessage(user_id,retryAfter);
			return
		elif RESPONSE == '[406 PHONE_NUMBER_INVALID]':
			delete(user_id);
			sendMessage(user_id,numberIsFalse);
			return
		elif RESPONSE == '[406 PHONE_PASSWORD_FLOOD]':
			delete(user_id);
			sendMessage(user_id,numberIsBanned);
			return
	...
	if success['_'] == 'SentCode':
		if success['type'] == 'SentCodeType.APP' or success['type'] == 'app':
			METHOD   = 'APP';
			sendMessage(user_id,doneApp);
		else:
			METHOD         = 'SMS';
			sendMessage(user_id,doneSMS);
		CODE_HASH   = success['phone_code_hash'];
		set(user_id,"status","verfiry");
		set(user_id,"code_hash",CODE_HASH);
		set(user_id,"method",METHOD);
		time.sleep(5);
		
	counter     = 0;
	while trueF:
		
		status      = get(user_id,"status");
		...
		if status == "verfiry" and code is not False and code != '':
			code_hash   = get(user_id,"code_hash");
			method         = get(user_id,"method");
			
			if method == 'APP':
				try:
					await app.sign_in(number, code_hash, str(code));
					sendMessage(user_id,doneLoginNotOut);
					...
					if RESPONSE == '[400 PHONE_CODE_INVALID]':
						sendMessage(user_id,errorCode);
						delete(user_id,"code");
					elif RESPONSE == '[400 PHONE_CODE_EXPIRED]':
						sendMessage(user_id,timeOutCode);
						delete(user_id,"code");
					elif RESPONSE == '[401 SESSION_PASSWORD_NEEDED]':
						sendMessage(user_id,sendCodeAuth);
						delete(user_id,"code");
						set(user_id,"status","auth");
					else:
						sendMessage(user_id,sendCodeAuth);
						delete(user_id);
						os.remove(f"sessions/{ses}.session");
				...
				try:
					try :
						...
						sendMessage(user_id,doneLogin);
						#sendMessage
						#await app.get_authorizations
						delete(user_id);
						exit();
						break;
						return
					...
					if RESPONSE == '[400 PHONE_CODE_INVALID]':
						...
						sendMessage(user_id,unknownError);
						delete(user_id);
						os.remove(f"sessions/{ses}.session");
						exit();
						break;
		...
		if status == "auth" and code is not False and code != '':
			try:
				...
				sendMessage(user_id,errorCodeAuth);
				delete(user_id,"code");
				print(ERROR_AUTH);
				
		## Check Time Out '___' ##
		if counter > 12:
			sendMessage(user_id,timeOutSession);
			delete(user_id);
