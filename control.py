...
opreats    = ['join','check','start','bot','Poll', 'likee','like','finish','leavesss','getN','reaction','getusers','getuser','joining','joinPro','leaveChat','View', 'JoinList', 'LeaveList'];
opreat      = sys.argv[1];
...
if opreat == 'bot':
	...
	for session in sessions:
		...
		AA     = count - Zero
		NAME_SESSION     = session.split('.')[0];
		RESPONSE	       = run_script(f"python3 addAccount.py bot {NAME_SESSION} {link} {UserBot} {III} {owner_id}");
		VIEWD    = ExtraTexts['botsend'].replace('++DONE++',str(Zero)).replace('++DS++',str(AA));
